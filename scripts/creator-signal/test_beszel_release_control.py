...
class ReleaseControlTest(unittest.TestCase):
    def manifest(self):
        ...
        return {"schema":"creator-signal.beszel-agent-release/v1","releaseTag":"v0.20.0-cs.1","sourceRevision":source,"upstream":{"repository":"henrygd/beszel","tag":"v0.20.0","revision":"a0dc19eacf6902d2f6934a3efd7ea7277198c40b"},"artifacts":{"linux-amd64":{"name":"beszel-agent_linux_amd64.tar.gz","sha256":"a"*64,"size":1,"binarySha256":"b"*64}},"provenance":{"type":"github-artifact-attestation"},"sbom":{"format":"spdx-json","name":"beszel-agent_linux_amd64.spdx.json","sha256":"c"*64},"qualification":{"forgejoRunId":"7","sourceRevision":source,"runnerLabel":"native-linux-amd64","nativeAgentSmoke":"pass","hubHandshake":"pass","metrics":"pass","hubTag":"v0.20.0","hubRevision":"a0dc19eacf6902d2f6934a3efd7ea7277198c40b"}}
    def test_policy_is_valid(self): validate(POLICY)
    def test_rejects_upstream_release_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            path = pathlib.Path(directory) / "manifest.json"; data = self.manifest(); data["releaseTag"] = "v0.20.0"; path.write_text(json.dumps(data))
